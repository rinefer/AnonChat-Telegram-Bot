...
Инициализирует платежи, интересы, планировщик подписок и запускает polling.

ВАЖНО: порядок импорта handlers имеет значение.
Telebot регистрирует обработчики в порядке импорта.
start.py импортируется ПОСЛЕДНИМ — его relay_text является запасным
обработчиком и не должен перехватывать команды из других модулей.
"""
import sqlite3
import time
import threading
